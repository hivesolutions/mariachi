...
void _vp_psy_init(vorbis_look_psy *p,vorbis_info_psy *vi,
  ...
  p->tonecurves=setup_tone_curves( vi->toneatt, (float)(rate*.5/n), n, (float)vi->tone_centerboost, (float)vi->tone_decay);
...
void _vp_offset_and_mix(vorbis_look_psy *p,
  ...
  for(i=0;i<n;i++){
    ...
    */
    ...
    de = (float)( 1.0-((val-coeffi)*0.005*cx) );
    ...
    de = (float)( 1.0-((val-coeffi)*0.0003*cx) );
      /* pro-rated attenuation:
...
static float dipole_hypot(float a, float b){
  if(a>0.){
    if(b>0.)return (float)sqrtf(a*a+b*b);
    if(a>-b)return (float)sqrtf(a*a-b*b);
    return (float)-sqrt(b*b-a*a);
  }
  if(b<0.)return (float)-sqrtf(a*a+b*b);
  if(-a>b)return (float)-sqrtf(a*a-b*b);
  return (float)sqrtf(b*b-a*a);
}
