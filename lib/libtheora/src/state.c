...
static int oc_sw_quad_top_left_frag(const oc_sb *_sb,int _quadi){
...
static void oc_sw_create_plane_mapping(oc_sb _sbs[],int _frag0,int _hfrags,
 int _vfrags){
  ...
  for(y=0;;y+=4){
    ...
    for(x=0;;x+=4,sb++){
      ...
      for(i=0;i<imax;i++){
        int j;
        for(j=0;j<jmax;j++){
          sb->map[SW_MAP[i][j][0]][SW_MAP[i][j][1]]=xfrag+j;
        }
        xfrag+=_hfrags;
      }
      /*Mark which quadrants of this super block lie within the image.*/
      for(quadi=0;quadi<4;quadi++){
        sb->quad_valid|=(oc_sw_quad_top_left_frag(sb,quadi)>=0)<<quadi;
      }
    }
    yfrag+=_hfrags<<2;
  }
...
static void oc_state_frarray_init(oc_theora_state *_state){
  ...
  for(pli=0;pli<3;pli++){
    oc_fragment_plane *fplane;
    fplane=_state->fplanes+pli;
    oc_sw_create_plane_mapping(_state->sbs+fplane->sboffset,
