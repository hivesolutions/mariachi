...
static int oc_sw_run_unpack(oggpack_buffer *_opb){
  long bits;
  int ret;
  /*Coding scheme:
       Codeword            Run Length
...
static int oc_dec_partial_sw_flags_unpack(oc_dec_ctx *_dec){
  oc_sb *sb;
  oc_sb *sw_end;
  long   val;
  int    flag;
  ...
  flag=(int)val;
  sb=_dec->state.sbs;
  sw_end=sb+_dec->state.nsbs;
  run_count=npartial=0;
  while(sb<sw_end){
    int full_run;
    run_count=oc_sw_run_unpack(&_dec->opb);
    ...
    while(--run_count>0&&sb<sw_end);
    if(full_run&&sb<sw_end){
      theorapackB_read1(&_dec->opb,&val);
      flag=(int)val;
    }
    else flag=!flag;
...
/*Decodes the bit flags for whether or not each non-partially-coded super
  ...
  Return: The number of partially coded super blocks.*/
static void oc_dec_coded_sw_flags_unpack(oc_dec_ctx *_dec){
  oc_sb *sb;
  oc_sb *sw_end;
  long   val;
  int    flag;
...
static void oc_dec_coded_flags_unpack(oc_dec_ctx *_dec){
  ...
  npartial=oc_dec_partial_sw_flags_unpack(_dec);
  if(npartial<_dec->state.nsbs)oc_dec_coded_sw_flags_unpack(_dec);
  if(npartial>0){
    theorapackB_read1(&_dec->opb,&val);
    flag=!(int)val;
  }
